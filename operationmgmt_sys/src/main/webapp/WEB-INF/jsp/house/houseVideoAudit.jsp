...
<body >
  ...
  <input type="button" value="查找" onclick="search()"/>
    ...
    <div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;" 
        url="${ctx}/houseVideo/getVideoAuditList.action"  idField="id" allowResize="false"
        sizeList="[20,30,50,100]" pageSize="20">
        <div property="columns">
            <!-- <div type="indexcolumn" style="display: none"></div> -->
            <div field="houseId" width="120"  headerAlign="center" align="center" allowSort="true">房源编号/小区编号</div>                            
    ...
    <script type="text/javascript">
     ...
     /*    grid.hideColumn("loginNameColumn");
        	...
        	if(field == "context"){
        		...
        		if(record.videostatus != 5){
        			...
        			if(videoTypes == 1){
        		 	...
        		 	e.cellHtml = "<a href='javascript:void(0)' onclick='passData(" + record.id + ",\"" + record.mobile+"\",\"" + estatename + "\",\"" + record.houseId  +"\")'>通过</a> | <a href='javascript:void(0)' onclick='openRejectedPage(" + record.id + ",\"" + record.mobile+"\",\"" + estatename + "\",\"" + record.houseId  +"\")'>驳回</a>";
        ...
        //驳回理由
        function openRejectedPage(id,mobile,estatename,houseId) {
        	
            var videoType = mini.get("videoType").getFormValue();//类型
   	     mini.open({
   	         url: "${ctx}/houseVideo/openRejectedPage.do?videoType="+videoType+"&id="+id+"&mobile="+mobile+"&estatename="+encodeURI(encodeURI(estatename))+"&houseId="+houseId,
   	         title: "驳回理由",
   	         width: 400, 
   	         height:200,
   	         ondestroy: function (action) {
        ...
        //通过
         function passData(id,mobile,estateName,houseId){
        	...
        	lc.mask("正在处理中,请稍后...");
        	$.ajax({
        		url : "${ctx}/houseVideo/updateAudit.action?videoType="+videoType+"&mobile="+mobile+"&id="+id+"&flag=0"+"&estateName="+encodeURI(encodeURI(estateName))+"&houseId="+houseId,
                cache : false,
                dataType : 'text',
      ...
      //      var videoType = mini.get("videoType").getFormValue();//类型
        	lc.mask("正在处理中,请稍后...");
        	$.ajax({
        		url : "${ctx}/houseVideo/updateAudit.action?videoType="+videoType+"&mobile="+mobile+"&id="+id+"&rejectreason="+encodeURI(encodeURI(rejectreason))+"&estateName="+encodeURI(encodeURI(estatename))+"&houseId="+houseId,
                cache : false,
                dataType : 'text',
                success : function(data) {
            		 if(data == 1){
