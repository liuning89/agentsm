 ...
 */
@Component
public class houseCountTownTask {
...
//	@Scheduled(cron="0 0/10 * * * ?")
	@Scheduled(cron="${houseCountReportTime}")
	public void pubCustomerTask(){
		logger.info("开始判断本机是否跑定时任务");
		if (isRunServer()) {
			//@Scheduled(cron="0 10 0 ? * *")
			logger.info("房源数据报表板块定时任务, 开始!");
			try {
				rulePublicService.runHouseCountMain();
	        }
	        catch (Exception e) {
	            logger.error("数据异常",e);
	        } 
			logger.info("房源数据报表板块定时任务, 结束!");
		}
	}
