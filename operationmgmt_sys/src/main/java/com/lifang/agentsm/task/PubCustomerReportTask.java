 ...
 */
@Component
public class PubCustomerReportTask {
...
//	@Scheduled(cron="0 0/10 * * * ?")
	@Scheduled(cron="${pubCustomerReportTime}")
	public void pubCustomerReportTask(){
		logger.info("开始判断本机是否跑定时任务");
		if (isRunServer()) {
			//@Scheduled(cron="0 10 0 ? * *")
			logger.info("公客报表定时任务, 开始!***************");
			try {
				rulePublicService.runReport();
	        }
	        catch (Exception e) {
	            logger.error("数据异常",e);
	        } 
			logger.info("公客报表定时任务, 结束!");
		}
	}
