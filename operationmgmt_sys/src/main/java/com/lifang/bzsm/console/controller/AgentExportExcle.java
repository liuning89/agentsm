...
@Controller
@RequestMapping(value = "/agentexportExcle")
public class AgentExportExcle extends BaseController {
    ...
    public String download(HttpServletRequest request, HttpServletResponse response, LfBzAgentReportRequest req) throws IOException {
        ...
        // 填充projects数据
        List<LfBzAgentReport> projects = createData(req, request.getSession());
        List<Map<String, Object>> list = createExcelRecord(projects,req);
        String columnNames[] = {"日期", "区域", "门店", "经纪人", "行程量", "带盘", "带客", "悟空找房", "网络", "其它", "新增总数", "发布", "钥匙", "速销", "实景","无效审核数量", "房源"
                , "客户", "营销指数", "分享房源次数", "分享房源套数", "好评", "差评"};
        String keys[] = {"日期", "区域", "门店", "经纪人", "行程量", "带盘", "带客", "悟空找房", "网络", "其它", "新增总数", "发布", "钥匙", "速销", "实景","无效审核数量", "房源"
                , "客户", "营销指数", "分享房源次数", "分享房源套数", "好评", "差评"};

        String columnNamessum[] = {"区域", "门店", "经纪人", "行程量", "带盘", "带客", "悟空找房", "网络", "其它", "新增总数", "发布", "钥匙", "速销", "实景","无效审核数量", "房源"
                , "客户", "营销指数", "分享房源次数", "分享房源套数", "好评", "差评"};
        String keyssum[] = {"区域", "门店", "经纪人", "行程量", "带盘", "带客", "悟空找房", "网络", "其它", "新增总数", "发布", "钥匙", "速销", "实景","无效审核数量", "房源"
                , "客户", "营销指数", "分享房源次数", "分享房源套数", "好评", "差评"};
    ...
    private List<LfBzAgentReport> createData(LfBzAgentReportRequest req, HttpSession session) {
        ...
        return response.getData() == null ? null : (List<LfBzAgentReport>) response.getData();
    }

    private List<Map<String, Object>> createExcelRecord(List<LfBzAgentReport> projects, LfBzAgentReportRequest req) {
        List<Map<String, Object>> listmap = new ArrayList<Map<String, Object>>();
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("sheetName", "sheet1");
        listmap.add(map);
        LfBzAgentReport project = null;
        for (int j = 0; j < projects.size(); j++) {
            ...
            mapValue.put("门店", project.getStoreOrgName());
            mapValue.put("经纪人", project.getAgentName());
            mapValue.put("手机", project.getAgentMobile());
            ...
            mapValue.put("调价数", project.getOrderEdit());
            mapValue.put("出价数", project.getOrderCreate());
            ...
            mapValue.put("无效审核数量", project.getInvalidReview());
                System.out.println(project.getInvalidReview());
